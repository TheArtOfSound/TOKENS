...
export const SOURCE_DISCLOSURES: SourceDisclosure[] = [
  {
    ...
    reads: 'ccusage claude daily --json (aggregated daily totals from local Claude Code logs)',
    directories: ['~/.claude (read by ccusage, not by this collector directly)'],
    extracts: ['date', 'model names', 'input/output/cache token counts', 'estimated cost'],
    discards: ['prompt text', 'response text', 'file paths', 'session identifiers', 'project names'],
  ...
  },
  {
    key: 'codex',
    name: 'Codex usage',
    reads: 'ccusage codex daily --json (aggregated daily totals from local Codex logs)',
    directories: ['~/.codex (read by ccusage, not by this collector directly)'],
    extracts: ['date', 'model names', 'input/output/cache token counts', 'estimated cost'],
    discards: ['prompt text', 'response text', 'file paths', 'session identifiers', 'project names'],
    evidenceClass: 'provider_reported',
    networkAccess: 'none (--offline uses a cached pricing table)',
